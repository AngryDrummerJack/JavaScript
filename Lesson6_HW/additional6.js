...
let string = 'revolution';
let cutString = function (arr, n) {
    let result = [];
    for (let i = 0; i < arr.length; i += n) {
        const resultElement = arr.slice(i,i+n);
        result.push(resultElement);
    }
    return result;
};
 document.write(cutString(string, 3));


// - Напишіть функцію delete_characters(str, length), яка повертає підрядок, що складається із зазначеної кількості символів.
let str = 'Каждый охотник желает знать';
// document.writeln(delete_characters(str, 7)); // Каждый
let delete_characters = function (chunk, length) {
    result = chunk.slice(0, length);
    return result;
};
...
let insert_dash = function (string) {
    let result = string
        .replaceAll(' ', '-')
        .toUpperCase();
    return result;
...
let clearNames = function (iName) {
    return  iName.replaceAll(/[)(-._]/g, ' ');
};
console.log(clearNames(n1));
console.log(clearNames(n2));
console.log(clearNames(n3));
//
